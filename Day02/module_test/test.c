...
{
	//*buf = 'X';
	num  = (num*350+num*num%33)*num;
	copy_to_user(buf, &num, sizeof num);
	return 4;
}
...
long myioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
	switch(cmd)
	{
	case 0:
		...
		break;
	case 1:
		//*(int*)arg = 250; //copy_to_user
		put_user(250, (int*)arg);//get_user()
	}
}
