...
ssize_t mywrite(struct file *filep, const char __user *ubufer, size_t size, loff_t *offset)
{
	...
	//kill signal
	kill_fasync(&mydev.fasync_queue, SIGIO, POLL_IN);
...
int myfasync(int fd, struct file *filep, int mod)
{
	//record
	return fasync_helper(fd, filep, mod, &mydev.fasync_queue);
}
