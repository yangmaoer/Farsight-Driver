...
{
	filep->private_data = (void*)iminor(inodep);
...
{
	int num = (int)filep->private_data;
...
int setup_mydev(void)
{
	...
	ret = alloc_chrdev_region(&mydev.devnum, 0, 4, "mycdev_test");
	if(ret < 0)
	{
	...
	ret = cdev_add(mydev.cdev, mydev.devnum, 4);
	if(ret)
	{	
		printk("devnum alloc failed !\n");
		goto add_failed;
